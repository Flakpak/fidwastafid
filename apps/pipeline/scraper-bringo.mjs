...
import { mapCategorie, rayonDepuisUrl } from "./_lib/categoriser.mjs";
...
function parseProducts(html, rayon = "") {
  ...
  $(".box-product").each((_, el) => {
    ...
    produits.push({
      ...
      categorie: mapCategorie(data.item_list_name, titre, rayon),
      // Stocké en base (migration 0018) : sans lui, la prochaine amélioration
      // du mapping devrait re-scraper pour retrouver ce que cette page vient
...
for (const listing of listings) {
  for (let pageNum = 1; pageNum <= MAX_PAGES; pageNum++) {
    ...
    let produits;
    try {
      produits = parseProducts(await fetchPage(url), rayonDepuisUrl(listing));
    } catch (err) {
