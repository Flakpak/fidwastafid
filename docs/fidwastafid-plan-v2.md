...
| 4 — Web | 🔶 en cours | Morceaux 1 (provisioning+auth) et 2 (feed SSR+slugify) faits et pushés. Morceau 3 (page deal, votes, commentaires, redirection slug canonique, état expiré) en cours. CSP par nonce désactivé en `next dev` uniquement (cause : casse l'hydratation sous HMR — confirmé sans lien avec la sécurité, la prod garde le CSP strict inchangé, voir commentaire dans `middleware.ts`) — conséquence : vérifier tout morceau ajoutant du JS client/ressource externe contre un build Docker, pas seulement `next dev`. |
| 5 — SEO | ☐ à faire | |
